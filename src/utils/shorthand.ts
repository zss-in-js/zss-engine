...
const DIRECT_LONGHANDS: Record<string, string[]> = {
  ...
  // `border-width` sets all four physical sides, so it resets the logical
  // width regardless of the writing mode (same for style / color)
  ...
  // `white-space` and `text-wrap` are siblings that overlap on text-wrap-mode
  'white-space': ['white-space-collapse', 'white-space-trim', 'text-wrap-mode'],
